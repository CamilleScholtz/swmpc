...
description: "Audit and enable security-oriented Xcode build settings, compiler warnings, static analysis, and Enhanced Security features."
---
...
When Xcode MCP tools (`XcodeGlob`, `XcodeGrep`, `XcodeRead`, etc.) are available, prefer them.

In standard CLI environments without Xcode MCP:
- Inspect build settings via:
  `xcodebuild -scheme <scheme> -destination 'generic/platform=macOS' -showBuildSettings -json | python3 .agents/skills/audit-xcode-security-settings/scripts/filter_build_settings.py --unhardened-only`
- Use standard CLI tools (`rg`, `fd`, `read`, `edit`).
...
- **XcodeGlob** for file discovery (when Xcode MCP is available)
- **XcodeGrep** for content search (when Xcode MCP is available)
- **XcodeRead** for file contents (when Xcode MCP is available)
- **XcodeLS** for directory listing (when Xcode MCP is available)
- **XcodeUpdate** for in-place edits of project-registered text files (xcconfig files, source files) — same `filePath` / `oldString` / `newString` (+ optional `replaceAll`) signature as the built-in `Edit` tool, but accepts Xcode workspace-relative paths. prefer XcodeUpdate when using Xcode MCP. **Do not** use `XcodeUpdate` / `Edit` / `plutil` to add or update `.entitlements` keys — use `AddEntitlement`.
