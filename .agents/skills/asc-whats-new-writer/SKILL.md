...
description: Generate engaging, localized App Store release notes (What's New) from git log, bullet points, or free text using canonical metadata under `./Store`. Optionally pairs with promotional text updates.
---
...
- Metadata pulled locally into canonical files via `asc metadata pull --app "APP_ID" --version "1.2.3" --dir "./Store"`. OR: user provides keywords manually.
...
# Bulk canonical-metadata push after writing ./Store/version/<version>/<locale>.json
asc metadata push --app "APP_ID" --version "1.2.3" --dir "./Store" --dry-run
asc metadata push --app "APP_ID" --version "1.2.3" --dir "./Store"
```
...
- The canonical `./Store` tree is what `asc metadata pull`, `asc metadata push`, and `asc metadata keywords ...` operate on.
