---
name: asc-aso-audit
description: Run an offline ASO audit on canonical App Store metadata under `./Store` and surface keyword gaps using Astro MCP. Use after pulling metadata with `asc metadata pull`.
---
...
- Metadata pulled locally into canonical files via `asc metadata pull --app "APP_ID" --version "1.2.3" --dir "./Store"`.
- If metadata came from `asc migrate export` or `asc localizations download`, normalize it into the canonical `./Store` layout before running this skill.
...
- For keyword-only follow-up after the audit, prefer the canonical keyword workflow:
  - `asc metadata keywords diff --app "APP_ID" --version "1.2.3" --dir "./Store"`
  - `asc metadata keywords apply --app "APP_ID" --version "1.2.3" --dir "./Store" --confirm`
  - `asc metadata keywords sync --app "APP_ID" --version "1.2.3" --dir "./Store" --input "./keywords.csv"` when importing external keyword research
