...
```bash
asc metadata pull --app "APP_ID" --version "1.2.3" --platform IOS --dir "./Store"
```
...
asc metadata pull --app "APP_ID" --app-info "APP_INFO_ID" --version "1.2.3" --platform IOS --dir "./Store"
```
...
```bash
asc metadata validate --dir "./Store" --output table
```

For subscription apps, include the extra Terms of Use / EULA heuristic:

```bash
asc metadata validate --dir "./Store" --subscription-app --output table
```
...
```bash
asc metadata push --app "APP_ID" --version "1.2.3" --platform IOS --dir "./Store" --dry-run --output table
```

Apply after the plan looks correct:

```bash
asc metadata push --app "APP_ID" --version "1.2.3" --platform IOS --dir "./Store"
```

Use `asc metadata apply` when the user wants the apply-named command shape for the same canonical files:

```bash
asc metadata apply --app "APP_ID" --version "1.2.3" --platform IOS --dir "./Store" --dry-run
asc metadata apply --app "APP_ID" --version "1.2.3" --platform IOS --dir "./Store"
```
...
```bash
asc metadata keywords diff --app "APP_ID" --version "1.2.3" --platform IOS --dir "./Store"
asc metadata keywords apply --app "APP_ID" --version "1.2.3" --platform IOS --dir "./Store" --confirm
```

For importing keyword research:

```bash
asc metadata keywords import --dir "./Store" --version "1.2.3" --locale "en-US" --input "./keywords.csv"
asc metadata keywords sync --app "APP_ID" --version "1.2.3" --platform IOS --dir "./Store" --input "./keywords.csv"
```
