    ...
    var body: some View {
        ...
        .actionFeedback(feedback)
            ...
            .task(id: album, priority: .medium) {
                ...
                artwork = await (try? album.artwork(fitting: Layout.RowHeight.album))?.image
            }
    }
}
...
private struct AlbumTitleText: View {
    ...
    var body: some View {
        Text(album.title)
            .font(.headline)
            .foregroundStyle(mpd.status.song?.isIn(album) ?? false ? AnyShapeStyle(.tint) : AnyShapeStyle(.primary))
            .lineLimit(2)
    }
}
