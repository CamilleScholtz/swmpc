...
@Observable final class DatabaseManager {
        ...
        async throws
    {
        ...
        let newMedia: [any Mediable]? = switch self.type {
        case .album:
            try await idle
                ? ConnectionManager.idle.getAlbums(sort: self.sort)
                : ConnectionManager.command().getAlbums(sort: self.sort)
        case .artist:
            try await idle
                ? ConnectionManager.idle.getArtists(sort: self.sort)
                : ConnectionManager.command().getArtists(sort: self.sort)
        case .song:
            try await idle
    ...
    /// matching the query against the specified fields. It parallelizes the search
    /// by dividing the media collection into chunks and processing them concurrently.
    ///
    /// - Parameters:
    ...
    func search(query: String, fields: Set<String>) async -> [any Mediable] {
        guard !query.isEmpty, let media else {
            return []
        }
                ...
                group.addTask {
                    chunk.filter { item in
                        ...
                        case let song as Song:
                            ...
                            (fields.contains("title") && song.title.lowercased().contains(lowercasedQuery)) ||
                                (fields.contains("artist") && song.artist.lowercased().contains(lowercasedQuery)) ||
                                (fields.contains("album") && song.album.title.lowercased().contains(lowercasedQuery))

                        case let album as Album:
                            (fields.contains("title") && album.title.lowercased().contains(lowercasedQuery)) ||
                                (fields.contains("artist") && album.artist.name.lowercased().contains(lowercasedQuery))

                        case let artist as Artist:
                            fields.contains("artist") && artist.name.lowercased().contains(lowercasedQuery)
